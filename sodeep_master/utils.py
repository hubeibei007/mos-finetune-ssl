...
def get_rank(batch_score, dim=0):
    ...
    batch_score_arr = batch_score.cpu().detach().numpy()
    rank1 = rankdata(batch_score_arr)
    rank1 = rank1 - 1
    rank1 = (rank1 * -1) + batch_score.size(dim)
    #print(rank1)
    rank1 = torch.tensor(rank1, device = batch_score.device)
    rank1 = rank1.float()
    rank1 = rank1 / batch_score.size(dim)
    #print(batch_score, rank, rank1)
    return rank1

def get_avg_rank(batch_score, dim=0):
    batch_score_arr = batch_score.cpu().detach().numpy()
    rank = rankdata(batch_score_arr)
    rank = rank - 1
    rank = (rank * -1) + batch_score.size(dim)
    print(rank)
    rank = torch.tensor(rank, device = batch_score.device)
    rank = rank / batch_score.size(dim)
    return rank
...
if __name__ == "__main__":
    #rankdata([0, 2, 3, 2])
    score = torch.tensor([0, 2, 3, 2])
    print(score)
    rank = get_rank(score, dim=0)
    print(rank)
    rank = get_avg_rank(score, dim=0)
    print(rank) 
